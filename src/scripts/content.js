...
function injectJS() {
  ...
  script.type = 'module';
  script.src = browser.runtime.getURL('resources/overlay/overlay.js');
  // script.defer = true;
  document.body.appendChild(script);
}

(async () => {
  const response = await browser.runtime.sendMessage({ type: "getBlockSettings" });
  ...
  if (isBlocked && enabled) {
    const [overlayHtml, overlayCss] = await Promise.all([
      fetch(browser.runtime.getURL("resources/overlay/overlay.html")).then(r => r.text()),
      fetch(browser.runtime.getURL("resources/overlay/overlay.css")).then(r => r.text()),
    ]);
    ...
    logo.src = browser.runtime.getURL('images/logo.svg');
